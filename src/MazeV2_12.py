...
class Level():
    ...
    def draw_build_wall_break_positions(self, build_debug):
        if build_debug == True:
            self.draw.draw(COLOURS["BLACK_VERY_LIGHT"], self.wall_break_positions[-1][0], self.wall_break_positions[-1][1])
...
class Character():
    ...
    def draw_player(self):
        self.draw.draw_tile(player_image, self.current_poistion[0] + ((GRID_SIZE - player_image.get_width())/2), self.current_poistion[1] + (GRID_SIZE - player_image.get_height()))
...
class Game():
    ...
    def build(self, build_debug):
        ...
        self.player_controller = Player_Controller(self.draw, self.level, self.character)
        self.ai_controller_01 = AI_Controller(self.draw, self.level, self.character, self.player_controller)
