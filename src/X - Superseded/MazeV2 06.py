...
class Level():
    ...
    def set_ends(self):
        ...
        for p in self.path_type.keys():
            path_directions = []
            for d in DIRECTIONS:
                direction = (p[0] + (d[0] * self.world.grid_size),
                             p[1] + (d[1] * self.world.grid_size))
    ...
    def set_route(self):
        route_list_A = []
        route_list_B = []
        route_list_A.append(world.maze_start_position)
        route_list_B.append(world.maze_finish_position)
        ...
        while run:
            ...
            if route_list_A[-1] == route_list_B[-1]:
                run = False
...
build = True
while build:
    world = World(width=WIDTH, height=HEIGHT, grid_size=20, draw=draw)
    world.MazeBuild()
...
player = Player(world, draw)
level = Level(world, draw)
